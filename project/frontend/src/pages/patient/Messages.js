...
const Messages = () => {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/patient/messages', {
  ...
  // Fetch assigned doctors function
  const fetchDoctors = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/patient/doctors/assigned', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      await Promise.all([fetchMessages(), fetchDoctors()]);
      setLoading(false);
    };
    loadData();
  }, [fetchMessages, fetchDoctors]);
  ...
  const handleEditMessage = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/patient/messages/${editingMessage._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const openFullMessage = async (message) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/patient/messages/${message._id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (response.ok) {
        ...
        if (fullMessage.recipient._id === currentUser.id && !fullMessage.isRead) {
          await fetch(`http://localhost:5000/api/patient/messages/${message._id}/read`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` }
          });
          fetchMessages();
        ...
        if (fullMessage.replies && fullMessage.replies.length > 0) {
          for (const reply of fullMessage.replies) {
            if (!reply.isRead && reply.sender._id !== currentUser.id) {
              await fetch(`http://localhost:5000/api/patient/messages/${message._id}/replies/${reply._id}/read`, {
                method: 'PUT',
                headers: { 'Authorization': `Bearer ${token}` }
              });
            }
  ...
  const handleSendReply = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/patient/messages/${selectedMessage._id}/reply`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleDeleteMessage = async (messageId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/patient/messages/${messageId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
          ...
          {doctors.length === 0 && (
            <div className="section-card" style={{ marginBottom: '2rem', background: '#fff3cd', borderColor: '#ffc107' }}>
              <p style={{ margin: 0, color: '#856404' }}>
                ℹ️ You can only send messages to doctors you have appointments with. Please book an appointment first.
              </p>
            </div>
          )}
            ...
            <div className="section-card" style={{ marginBottom: '2rem' }}>
              ...
              <form onSubmit={handleSendMessage}>
                <div className="form-group">
                  <label className="form-label">To (Assigned Doctor) *</label>
                  <select
                  ...
                  {doctors.length === 0 && (
                    <small style={{ color: '#dc3545', marginTop: '0.25rem', display: 'block' }}>
                      No assigned doctors. Book an appointment to send messages.
                    </small>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                  ...
                  <button 
                    ...
                    style={{ 
                      padding: '0.75rem 1.5rem', 
                      background: doctors.length === 0 ? '#ccc' : '#10b981', 
                      color: 'white', 
                      border: 'none', 
                      borderRadius: '8px', 
                      cursor: doctors.length === 0 ? 'not-allowed' : 'pointer', 
                      fontWeight: '600' 
                    }}
                  >
                    📤 Send Message
                ...
                <div style={{ padding: '1.5rem', background: '#f9fafb', borderRadius: '8px', marginBottom: '1.5rem' }}>
                  ...
                  <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '0.85rem', color: selectedMessage.isRead ? '#10b981' : '#ef4444', fontWeight: '600' }}>
                      {selectedMessage.isRead ? `✓ Seen Successfully on ${formatDate(selectedMessage.readAt)}` : '✗ Not Seen Yet'}
                    </span>
                      ...
                      return (
                        <div key={index} style={{ padding: '1rem', background: isSentByMe ? '#e8f5e9' : '#eff6ff', borderRadius: '8px', marginBottom: '1rem', marginLeft: '2rem' }}>
                          ...
                          <div style={{ fontSize: '0.75rem', color: reply.isRead ? '#10b981' : '#ef4444', marginLeft: '2.5rem', marginTop: '0.5rem', fontWeight: '600' }}>
                            {reply.isRead ? `✓ Seen by ${isSentByMe ? 'Doctor' : 'You'}` : '✗ Not Seen Yet'}
                          </div>
                        </div>
                      );
                    })}
            ...
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
              <div style={{ background: 'white', borderRadius: '12px', maxWidth: '600px', width: '100%', padding: '2rem' }}>
                ...
                <p style={{ color: '#6c757d', marginBottom: '1rem' }}>
                  You can only edit messages that haven't been seen by the doctor yet.
                </p>
                <textarea
            ...
            {messages.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {messages.map((msg) => {
                  ...
                  const isUnread = !isSentByMe && !msg.isRead;
                  const hasUnreadReplies = msg.replies && msg.replies.some(r => !r.isRead && r.sender._id !== currentUser.id);
                  ...
                  return (
                    <div
                      ...
                      style={{
                        padding: '1.5rem',
                        border: isUnread || hasUnreadReplies ? '2px solid #4a9eff' : '1px solid #e5e7eb',
                        borderRadius: '8px',
                        background: isUnread || hasUnreadReplies ? '#eff6ff' : 'white',
                        cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                      onClick={() => openFullMessage(msg)}
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                          ...
                          <div>
                            <div style={{ fontWeight: '700', fontSize: '1rem' }}>
                              {isSentByMe ? `To: Dr. ${otherPerson.name}` : `From: Dr. ${otherPerson.name}`}
                            </div>
              ...
              <div style={{ textAlign: 'center', padding: '4rem' }}>
                ...
                <h3 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No Messages</h3>
